...
//  Aluminium
//
//  Created by Hayden Chalin on 2/18/26.
//
...
enum DebugLights {
    ...
    static func debugDrawDirection(
        ...
        for index in -count ..< count {
            let lineSpaceMultiplier: Float = 0.4
            let spacing = Float(index) * lineSpaceMultiplier
//            print("Index: \(index), Value: \(spacing)")
            // Line starting point
            let startPoint = float3(spacing, 0, spacing)
            let endPoint = float3(direction.x + spacing, direction.y, direction.z + spacing)
