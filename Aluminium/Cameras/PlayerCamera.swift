...
struct PlayerCamera : Camera {
    ...
    mutating func update(dT: Float) {
        let transform = updateInput(dT: dT) // Returns a transform
        rotation += transform.rotation // update the rotation of the matrix
        position += transform.position // update the position of the matrix
